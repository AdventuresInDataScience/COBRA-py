...
from rbdpo.search.nevergrad_runner import run_nevergrad
from rbdpo.search.space import build_config_space
from rbdpo.validation.walk_forward import walk_forward_validate
...
def run(data_path: str, config_path: str | None, output_path: str | None, objective: str | None, seed: int | None, budget: int | None) -> None:
    ...
    if bool(cfg["validation"].get("walk_forward", True)):
        def optimise_fold(train_df, full_cfg):
            fold_cache = precompute_all(train_df, DEFAULT_REGISTRY, n_jobs=1)
